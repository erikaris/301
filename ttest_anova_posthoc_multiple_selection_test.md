...
### Illustration: same data, two ways of presenting it

Imagine three dose groups (Control, Low, High), each with **3 biological replicates** (3 separate animals/dishes per group — n=3 per group):

| Group | Replicate 1 | Replicate 2 | Replicate 3 | Mean |
|---|---|---|---|---|
| Control | 93 | 95 | 97 | 95 |
| Low dose | 78 | 80 | 82 | 80 |
| High dose | 48 | 50 | 52 | 50 |

**Poor practice:**
- Figure shows only 3 bars at heights 95, 80, 50 — the mean of each group. The 3 underlying replicate values are invisible; the reader has no way to judge spread or reliability from the figure alone.
- Every possible pairwise comparison is annotated: Control-vs-Low, Control-vs-High, *and* Low-vs-High — 3 brackets with asterisks, even though Low-vs-High wasn't part of the actual research question.
- No *n*, test, or correction method stated anywhere on the figure.

**Good practice:**
- The same 3 bars are shown, but each bar also has its 3 individual replicate values plotted as dots scattered around the bar (e.g., dots at 93, 95, 97 around the Control bar). This is what "showing biological replicates" means in practice — the mean is still there, but the underlying data and its spread are visible too.
- Only 2 comparisons are annotated: Control-vs-Low and Control-vs-High — because the actual hypothesis was "does each dose differ from control," not "do all three groups differ from each other." Low-vs-High is correctly left unannotated since it wasn't a question being tested.
- Legend states: "n = 3 biological replicates per group; Dunnett's test vs. control."

**The key shift:** the bars alone only ever show you the average. Adding the dots shows the reader the actual spread of your biological replicates — which is exactly what lets them judge whether a difference looks real or could just be noise. And restricting the brackets to only the comparisons tied to your hypothesis (rather than every mathematically possible pair) is what avoids over-interpretation (Section 6).
